...
const defaultRelays = env.defaultRelays.map((url) => ({
    url,
    read: true,
    write: true,
}));
export const useRelayStore = create()(persist((set) => ({
    ...
    addRelay: (url, read = true, write = true) => {
        ...
        set((s) => {
            if (s.relays.some((r) => r.url === normalised))
                return s;
    ...
    toggleRead: (url) => {
        set((s) => ({
            relays: s.relays.map((r) => r.url === url ? { ...r, read: !r.read } : r),
        }));
    },
    toggleWrite: (url) => {
        set((s) => ({
            relays: s.relays.map((r) => r.url === url ? { ...r, write: !r.write } : r),
        }));
    },
