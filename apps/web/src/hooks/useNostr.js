...
export function useNostr() {
    ...
    useEffect(() => {
        ...
        if (privateKey) {
            manager.subscribe("inbox", [{ kinds: [1059], "#p": [publicKey], limit: 100 }], (event) => {
                const msg = receiveDm(event, privateKey, publicKey);
                if (!msg)
                    return;
                const peerPubkey = msg.direction === "received" ? msg.senderPubkey : msg.recipientPubkey;
    ...
    const sendMessage = useCallback(async (recipientPubkey, text, file) => {
        if (!privateKey || !publicKey) {
            throw new Error("Private key required to send messages");
        }
        const manager = managerRef.current;
        if (!manager)
            throw new Error("Relay manager not initialised");
        let fileAttachment;
        if (file) {
